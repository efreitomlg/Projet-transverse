...
router.get('/researchname', async (req,res)=> {
  const name=req.body.name
    results = await client.query({
    text: "SELECT * FROM jouets WHERE name =$1",
    values: [name]
  })
  res.json(results.rows)
})

router.post('/invente', async (req, res) => {
  const name = req.body.name
  const description = req.body.description
  //const image = req.body.image
  /*const sql = */
  const result = await client.query({
    text: "SELECT name FROM jouets WHERE name =$1",
    values: [name]
  })

  console.log("je suis un checkpoint")
  if (result.rows.length > 0) {
    res.status(401).json({
      message: 'toy already exist'
    })
    return
  }
  
  await client.query({
    text: `INSERT INTO jouets(name, description)
    VALUES ($1, $2)
    `,
...
.delete('/deletetoy',async (req, res) =>{

  const name = req.body.name
  console.log('allezfautquecamercheputaing')
  await client.query({
  text: "DELETE FROM jouets WHERE name =$1",
  values: [name]
})
res.send('done')
})

router.get('/retrievetab', async (req, res) => {
const result = await client.query({
  text: 'SELECT * FROM jouets'
})
